...
u_int16_t get_Ether_Type(char *pointer)
{
    ...
    u_int8_t slb_eth = 0;
    u_int8_t lb_eth = 0;
    slb_eth = *(pointer + skip);// second last byte of ETH layer
    lb_eth = *(pointer + skip + 1);// last last byte of ETH layer
    return (slb_eth * 256) +lb_eth;
}
...
for(int i=0;i<nb_rx;i++){
  ...
  u_int8_t source_mac_address[RTE_ETHER_ADDR_LEN];
  u_int8_t destination_mac_address[RTE_ETHER_ADDR_LEN];
  struct ipv4_hdr     *pIP4Hdr;
  struct ipv6_hdr     *pIP6Hdr;
  ...
  if (is_ipv4(next_proto)){
     pIP4Hdr = rte_pktmbuf_mtod_offset(m, struct ipv4_hdr*, sizeof (struct ether_hdr));
