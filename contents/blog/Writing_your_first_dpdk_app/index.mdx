...
performance then you might need to look into fast pacekt processing libraries
such as DPDK.
...
With DPDK you bind the interface with the drivers provided, after which when the
packet is received the application can directly access it. 

In short DPDK allows to bypass the kernel and receiver and process the packets
directly in user space.

Datapath ususally entails 10s to 100s of Gigs per second on an interface. If you
want to write code dealing with such a traffic DPDK would be a fair choice.


## What Do you need?

In order to follo along with the material you will need the following:


- A linux based system:
    - 2 Intel processors, each with 8 cores, 8 GB RAM 
    - [DPDK compatible NIC](http://core.dpdk.org/supported/)
- Some knowledge of C
...
Some Useful Links
- [Programmers Guide](http://doc.dpdk.org/guides/prog_guide/intro.html)
- [API Documentation](http://doc.dpdk.org/api/)
...
```
Now that the dependencies have been taken care of lets get the DPDK tarfile
and build it.
...
2. isolates certain cpus from kernel scheduling so that we can dedicate those for
our application.
...
- the ouptut should be something like
...
```bash
test:~/dpdk-stable-20.11.1/usertools$ ./dpdk-devbind.py -s
...
- phew!
- We have finally set up the dpdk and loaded the NIC.
- In the next one we will write C code to receive and transmit the packets
using DPDK.
...
`l2fwd`.
- you cand read about **L2FWD** [here](https://doc.dpdk.org/guides/sample_app_ug/l2_forward_real_virtual.html) 